let Games = [
    
    {
        ID: "1",
        Name: "Valorant",
        Price: "Free",
        Subtitle: "เป็นเกมแนว FPS โดยจะแบ่งเป็น 2 ทีม แต่ละทีมจะมี 5 คน โดยจะทำทุกวิถีทางเพื่อให้ภารกิจของฝั่งตัวเองสำเร็จ ฝั่งแรกเป็นคนวางระเบิด เพื่อนำสารเขาสู่อีกมิติ และอีกฝั่งจะเป็นคนพยายามกันไม่ให้ฝ่ายวางระเบิดได้สำเร็จหรือ กู้ระเบิดเพื่อหยุดยั้งไม่ให้ระเบิดได้",
        tage: ["First-Person Shooter", "Online", "Sci-Fi"],
        link: "https://playvalorant.com/th-th/?gad=1&gclid=CjwKCAjw1t2pBhAFEiwA_-A-NCd1PBqJmYQ5VR6qhloNrSsIT5gPOg3y62XqZxgdIdEt8Ep2N6-PlxoCAxAQAvD_BwE&gclsrc=aw.ds",
        Logo: "./Icon/Valorant.jpg",
        Image: ["./Icon/Valorant01.webp"],
        Make: "Riot games",
        info: "Add info me please."
        ,MaxNumPlayer: 100000000
    },
    {
        ID: "2",
        Name: "League of Legends",
        Price: "Free",
        Subtitle: "เป็นเกม Online แนว MOBA แบ่งเป็น 2 ทีม โดยทั้ง 2 ทีมจะต้องทำลายป้อมใหญ่สุดที่เรียกว่า Nexus โดยเมื่อทีมไหนเป็นคนทำลาย Nexus ของอีกฝ่ายได้ก่อนฝ่ายนั้นเป็นฝ่ายชนะ",
        tage: ["MOBA", "Online"],
        link: "https://www.leagueoflegends.com/th-th/",
        Logo: "./Icon/LeagueOfLegends.jpg",
        Image: ["./Icon/LeagueOfLegends01.avif", "./Icon/LeagueOfLegends02.jpg"],
        Make: "Riot games",
        info: "เกมมีผู้พัฒนาเป็น Riot Games ค่ายเกมที่มีชื่อเสียงด้านเพลง, MV, Movie และการแข่งขันเกมต่างๆ โดยเกมนี้จะมีจุดเด้นเรื่อง เนื้อเรื่อง เกมเพลย์ เกมนี้จะมีเนื้อเรื่องของตัวละครทุกตัว และคุณสามารอ่านเนื้อเรื่องได้ผ่านเว็บไซย์หลัก"
        ,MaxNumPlayer: 110000000
    },
    {
        ID: "3",
        Name: "Terraria",
        Price: "220.00 Bath",
        Subtitle: "ดินแดนแห่งการผจญภัย! ดินแดนแห่งความพิศวง! ดินแดนที่คุณจะได้สร้าง ป้องกัน และ เพลิดเพลินไปกับมัน! ตัวเลือกของคุณใน Terraria มีไม่สิ้นสุด คุณเป็นเกมเมอร์ประเภทนิ้วไวหรือเปล่า หรือว่าเป็นก่อสร้างระดับเทพ เป็นนักสะสม? เป็นนักสำรวจ? มีบางสิ่งเพื่อคุณทุกคน",
        tage: ["SandBox", "Online", "Offline", "Adventure"],
        link: "https://store.steampowered.com/app/105600/Terraria/",
        Logo: "./Icon/Terraria.jpg",
        Image: ["./Icon/Terraria01.jpg", "./Icon/Terraria02.jpg"],
        Make: "Re-Logic",
        info: "เริ่มต้นด้วยการสร้างที่หลบภัยง่ายๆ จากนั้นก็เริ่มขุดหาสินแร่ หรือทรัพยากรอื่นๆ ค้นพบ และประดิษฐ์ อาวุธเวทมนต์ อาวุธระยะไกล หรืออาวุธประชิดตัวมากหมายหลายหลาก รวมไปถึงชุดเกราะต่างๆ แล้วใช้พวกมันต่อสู้กับศัตรูต่างๆ นับร้อย สักพักคุณก็จะสามารถมุ่งหน้าไปต่อกรกับบอสตัวร้าย หรือจะออกไปตกปลา ขี่สัตว์ ตามหาเกาะลอยฟ้า สร้างบ้านให้ NPC และอื่นๆ อีกเยอะแยะ"
        ,MaxNumPlayer: 1000000
    },
    {
        ID: "4",
        Name: "Portal",
        Price: "220.00 Bath",
        Subtitle: "เมื่อคุณตื่นจากการหลับไหล ในห้องการทดลอง ห้องหนึ่ง และคุณต้องหาทางออกเพื่อเป็นอิสระ จากการทดลอง โดยจะมีปืนประตูมิติ โดยปืนนี้จะไม่มีดาเมจ แต่สามารถพาคุณไปจุดที่คุณต้องการได้ และคุณต้องใช้ปืนนี้เพื่อออกจากที่นี้!!",
        tage: ["Puzzle", "Offline", "Adventure", "Sci-Fi"],
        link: "https://store.steampowered.com/app/400/Portal/",
        Logo: "./Icon/Portal.jpg",
        Image: ["./Icon/Portal01.webp"],
        Make: "Valve",
        info: "เกมนี้เป็นแนวแก้ไขปริศณา ผจภัยในโลกอนาคตและคุณเป็นตัวเอกในเกมนี้ เป้าหมายในกมคือการออกจากสถานที่นี้"
        ,MaxNumPlayer: 10000
    },
    {
        ID: "5",
        Name: "Titanfall",
        Price: "799.00 Bath",
        Subtitle: "Titanfall เป็นเกมแนว FPS ฟอร์มยักษ์ที่ค่อนข้างแปลกจากเกม FPS อื่นๆทั่วไปตรงที่ว่า เกมนี้จะไม่มีโหมดการเล่นตามเนื้อเรื่องแบบคนเดียวเลย จะมีแต่การเล่นแบบ Multiplayer เท่านั้น ซึ่งจะชดเชยด้วยโหมดต่างๆมากมาย",
        tage: ["First-Person Shooter", "Offline", "Adventure", "Sci-Fi"],
        link: "https://store.steampowered.com/app/1237970/Titanfall_2/",
        Logo: "./Icon/Titanfall-2.png",
        Image: ["./Icon/Titanfall01.webp","./Icon/Titanfall02.webp"],
        Make: "Respawn Entertainment",
        info: "ที่จะมีทั้งแบบทั่วไปเช่น Capture The Flag, Deathmatch เป็นต้น และมีโหมดแปลกๆเช่น Last Titan Standing ที่จะแข่งกันว่าฝ่ายใดเหลือ Titan รอดเป็นคนสุดท้ายเป็นฝ่ายชนะ"
        ,MaxNumPlayer: 10000
    },
    {
        ID: "6",
        Name: "Marvel snap",
        Price: "free",
        Subtitle: "MARVEL SNAP การ์ดเกมจากค่ายหนังสุดดัง อย่าง Marvel!!",
        tage: ["Card Collectible Game", "Online"],
        link: "https://store.steampowered.com/app/1997040/MARVEL_SNAP/",
        Logo: "./Icon/marvelSnap.jpg",
        Image: ["./Icon/marvelSnap01.jpg"],
        Make: "Nuverse",
        info: "MARVEL SNAP จาก Nuverse และ Second Dinner เป็นเกมแนว Card Collectible Game (CCG) ที่เราจะได้จัดเด็คเหล่าฮีโร่และวายร้ายจากจักรวาล MARVEL เข้าดวลกับคู่แข่ง! โดยเกมนี้โดดเด่นในด้านเกมเพลย์ที่ง่าย มีลูกเล่นสุดแหวกแนว และจบเร็วมากเพียงแค่ 3 นาทีเท่านั้น!"
        ,MaxNumPlayer: 150000
    },
    {
        ID: "7",
        Name: "MY HERO ULTRA RUMBLE",
        Price: "Free",
        Subtitle: "My Hero Ultra Rumble เกมฟรีจากมังงะและอนิเมะชื่อดังของ โคเฮ โฮริโคชิ อย่าง มายฮีโร่ อคาเดเมีย ในรูปแบบ Battle Royale",
        tage: ["Adventure", "Sci-Fi", "Battle Royale", "Online"],
        link: "https://store.steampowered.com/app/1607250/MY_HERO_ULTRA_RUMBLE/",
        Logo: "./Icon/myHeroUltraRumble.jpg",
        Image: ["./Icon/myHeroUltraRumble01.avif","./Icon/myHeroUltraRumble02.jpg"],
        Make: "Byking Inc.",
        info: "ใน My Hero Ultra Rumble ผู้เล่นจะถูกแบ่งเป็นทีมละ 3 คน และเข้าร่วมในการต่อสู้แบบ 8 ทีม ผู้เล่นต้องสื่อสาร, วางกลยุทธ์ และประสานอัตลักษณ์กันอย่างมีประสิทธิภาพ เพื่อเป็นทีมสุดท้ายที่ยืนอยู่ท่ามกลางผู้เล่น 24 คน การต่อสู้แบ Battle Royale เกิดขึ้นภายใต้ฉากหลังอันมีชีวิตชีวาของจักรวาล มายฮีโร่ อคาเดเมีย ที่ซึ่งฮีโร่และวายร้ายปะทะกันในการต่อสู้ครั้งยิ่งใหญ่ สร้างประสบการณ์การเล่นเกมอันตื่นเต้น แฟน ๆ สามารถเลือกตัวละครจาก MHA มาลงสังเวียนได้ โดยแต่ละตัวจะแบ่งออกเป็นหนึ่งในห้า Archetype ที่แตกต่างกัน ได้แก่ Strike, Assault, Rapid, Technical และ Support"
        ,MaxNumPlayer: 1000000
    },
    {
        ID: "8",
        Name: "Cyberpunk 2077",
        Price: "2,482.16 ฿",
        Subtitle: "ในเกม Cyberpunk 2077 เราจะได้รับบทเป็นตัวละครที่มีชื่อว่า วี (V) ในเกมนี้เต็มไปด้วยข้อมูลมากมายที่รอให้เราไปสำรวจและหาคำตอบให้ได้ และปัญหาที่สำคัญที่สุดในเกม คือการทำภารกิจหาคำตอบว่า จอห์นนี่ ซิลเวอร์แฮนด์ (Johnny Silverhand) นั้นคือใคร เขามีวีรกรรมอะไรที่สำคัญต่อNight City บ้าง และตอนนี้เขาไปอยู่ที่ไหนในโลกแห่งนี้",
        tage: ["Offline", "First-Person Shooter", "Sci-Fi"],
        link: "https://store.steampowered.com/app/1091500/Cyberpunk_2077/",
        Logo: "./Icon/Cyberpunk2077.jpg",
        Image: ["./Icon/Cyberpunk207701.jpg"],
        Make: "Nuverse",
        info: "Cyberpunk 2077 เต็มไปด้วยภารกิจย่อยมากมาย ซึ่งภารกิจเหล่านี้จะพาคุณไปรู้จักกับทุกมุมของ Night City และเป็นการแนะนำเมืองหลักในการดำเนินเนื้อเรื่องของเกมนี้ได้อย่างชาญฉลาด การเล่นภารกิจย่อยทำให้ผู้เล่นเพลิดเพลินและตื่นเต้นที่จะได้ศึกษาเรื่องราวของ Night City แต่เมื่อทำภารกิจไปได้สักระยะ ก็อาจทำให้เรารู้สึกเบื่อหน่ายเล็กน้อย เพราะภารกิจย่อยส่วนใหญ่ไม่ค่อยสอดคล้องกับเนื้อเรื่องหลักสักเท่าไหร่นัก"
        ,MaxNumPlayer: 900000
    },
    {
        ID: "0",
        Name: "Apex legends",
        Price: "Free",
        Subtitle: "เป็นเกมแนว FPS, Battle Royale โดยจะมีทีม 20 ทีม แต่ละทีมจะมี 3 คน และทำทุกวิธีเพื่อให้รอดจนเหลือทีสุดท้าย โดยจะมีตัวละครให้เลือกมากมายแต่ละตัวก็จะมีความสามารถแตกต่างกัน แต่ทุกตัวะครมีจุประสงค์เดียวกันคือ การเป็นที่ 1",
        tage: ["First-Person Shooter", "Online", "Battle Royale", "Sci-Fi"],
        link: "https://store.steampowered.com/app/1172470/Apex_Legends/",
        Logo: "./Icon/ApexLegends.png",
        Image: ["./Icon/ApexLegends01.jpg","./Icon/ApexLegends02.jpg"],
        Make: "Respawn Entertainment",
        info: "จัดจำหน่ายโดยอิเล็กทรอนิกอาตส์ ตัวเกมถูกเซ็ตฉากอยู่ในจักรวาลเดียวกับเกม Titanfall ตัวเกมเปิดให้เล่นสำหรับเครื่องPS4, Windows และ Xbox เปิดตัวครั้งแรกในวันที่ 4 กุมภาพันธ์ 2019 Apex legends สร้างความแตกต่างจากเกมแบตเทิลรอยัลเกมอื่น ๆ ด้วยการยืมแนวคิดของเกมแนวฮีโร่ชูตเตอร์ กับผู้เล่นเป็นตัวละครในตำนานแต่ละคนมีทักษะและความสามารถที่ไม่เหมือนใคร และต้องการให้ผู้เล่นทำงานเป็นทีมสามคนโดยมีความสามารถในการตอบโต้กับเพื่อนในขณะที่ Respawn ได้ทำงานเกี่ยวกับความพยายามในการพัฒนาภาคต่อที่สองในซีรีส์ Tintanfal"
        ,MaxNumPlayer: 961353
    },
    {
        ID: "9",
        Name: "Cyberdrome",
        Price: "Free",
        Subtitle: "None",
        tage: ["Offline", "First-Person Shooter", "Sci-Fi", "Adventure"],
        link: "https://store.steampowered.com/app/934850/Cyberdrome/",
        Logo: "./Icon/Cyberdrome.png",
        Image: ["./Icon/Cyberdrome01.jpg"],
        Make: "Nuverse",
        info: "Add info me please."
        ,MaxNumPlayer: 91000
    },
    {
        ID: "10",
        Name: "ARK: Survival Evolved",
        Price: "315.00 ฿",
        Subtitle: "None",
        tage: ["Offline", "First-Person Shooter", "Sci-Fi", "Adventure"],
        link: "https://store.steampowered.com/app/346110/ARK_Survival_Evolved/",
        Logo: "./Icon/ARK.jfif",
        Image: ["./Icon/ARK01.avif"],
        Make: "Nuverse",
        info: "Add info me please."
        ,MaxNumPlayer: 90000
    }
];